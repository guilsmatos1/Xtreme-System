# Graph Report - plankton  (2026-07-14)

## Corpus Check
- 143 files · ~53,567 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1324 nodes · 2584 edges · 198 communities (106 shown, 92 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 200 edges (avg confidence: 0.74)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `3b549644`
...
- `_ctx_list_custos()` --references--> `CustoVeiculo`  [EXTRACTED]
  bases/xtreme_system/api/routes/ui_routes/custos_veiculos.py → components/xtreme_system/custo_veiculo/core.py
...
Cohesion: 0.07
Nodes (66): MonkeyPatch, Path, TestClient, _admin_headers(), client(), _criar_cliente(), _FakeFile, _FakeUpload (+58 more)
...
Cohesion: 0.14
Nodes (22): Base, create(), delete(), get(), list_all(), Perfil, PerfilCreate, PerfilRead (+14 more)
...
### Community 6 - "core.py"
Cohesion: 0.08
Nodes (52): TipoCliente, create(), delete(), get(), get_by_placa(), list_all(), Decimal, Session (+44 more)
...
Cohesion: 0.09
Nodes (77): Path, UploadFile, Shared helpers for HTMX route modules., Retorna mensagem de erro do primeiro arquivo inválido, ou None.      Lote inteir, _remover_upload(), _uploaded_file_path(), _uploads_cliente_dir(), _uploads_compra_dir() (+69 more)
...
Cohesion: 0.08
Nodes (51): _baixar_contrato_venda(), _criar_venda(), _ctx_form_venda(), _erro_venda(), _gerar_contrato_pdf(), _ok_venda(), _parse_venda_form(), Any (+43 more)

### Community 12 - "core.py"
Cohesion: 0.08
Nodes (36): _atomic_write(), _conflict_form_response(), _create_with_hook(), CrudModule, _delete_with_hook(), Any, FastAPI, HTMLResponse (+28 more)

### Community 13 - "usuarios.py"
Cohesion: 0.33
Nodes (11): create(), CustoVeiculo, CustoVeiculoCreate, CustoVeiculoRead, CustoVeiculoUpdate, delete(), get(), list_all() (+3 more)
...
Cohesion: 0.25
Nodes (8): BaseModel, ClienteRead, DocumentoContratoVendaRead, DocumentoProcuracaoRead, VendaRead, DocumentoVeiculoRead, ImagemDocumentoClienteRead, ImagemVeiculoRead
...
Cohesion: 0.07
Nodes (50): change_password(), create(), delete(), get(), get_by_username(), list_all(), Session, Usuário: enum de papel, model, schemas e CRUD. (+42 more)
...
Cohesion: 0.05
Nodes (63): Engine, client(), TestClient, API auth: login, proteção por autenticação e por papel., Create/trocar-senha/delete de usuário pela API JSON devem atribuir o admin     c, test_admin_escreve(), test_admin_nao_pode_se_autoexcluir(), test_admin_pode_excluir_outro_admin() (+55 more)
...
## Knowledge Gaps
- **112 isolated node(s):** `Visão geral`, `Estrutura Polylith`, `App FastAPI`, `Dependências de autenticação`, `Endpoints JSON` (+107 more)
...
- **Why does `Base` connect `Form` to `core.py`, `create_test_engine`, `core.py`, `core.py`, `API - Xtreme Motors`, `core.py`, `auditoria.py`, `Response`, `core.py`, `core.py`, `core.py`, `Setup`?**
  _High betweenness centrality (0.061) - this node is a cross-community bridge._
- **Why does `_found()` connect `veiculos.py` to `_found`, `core.py`, `setup.py`, `API - Xtreme Motors`, `5c6914b729c6_index_fks_veiculo.py`, `test_venda_whatsapp.py`?**
  _High betweenness centrality (0.054) - this node is a cross-community bridge._
- **Why does `create_test_engine()` connect `core.py` to `test_venda_whatsapp.py`, `core.py`, `setup.py`?**
  _High betweenness centrality (0.054) - this node is a cross-community bridge._
- **Are the 42 inferred relationships involving `_found()` (e.g. with `_veiculos_comprador_modal()` and `_veiculos_vendedor_modal()`) actually correct?**
  _`_found()` has 42 INFERRED edges - model-reasoned connections that need verification._
