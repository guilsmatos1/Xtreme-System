# Graph Report - leatherjacket  (2026-07-14)

## Corpus Check
- 148 files · ~57,351 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1393 nodes · 2784 edges · 201 communities (112 shown, 89 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 181 edges (avg confidence: 0.73)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `de460dfa`
...
- `_erro_lancamento()` --references--> `LancamentoInvestimento`  [EXTRACTED]
  bases/xtreme_system/api/routes/ui_routes/investidores.py → components/xtreme_system/caixa/core.py

## Import Cycles
- None detected.

## Communities (201 total, 89 thin omitted)
...
### Community 5 - "core.py"
Cohesion: 0.08
Nodes (51): AdminUser, Auditoria, Base, confirmar_fechamento_venda(), criar_usuario(), deletar_usuario(), _guard_lancamento_veiculo(), health() (+43 more)

### Community 6 - "core.py"
Cohesion: 0.09
Nodes (50): OrigemLancamento, TipoCliente, create(), delete(), get(), get_by_placa(), list_all(), Decimal (+42 more)

### Community 7 - "setup.py"
Cohesion: 0.08
Nodes (25): Cookie, Exception, JSONResponse, oauth2_scheme, get_current_user(), get_ui_user(), _NaoAdminError, _NaoAutenticadoError (+17 more)
...
Cohesion: 0.08
Nodes (82): Path, Shared helpers for HTMX route modules., _remover_upload(), _uploaded_file_path(), _uploads_cliente_dir(), _uploads_compra_dir(), _uploads_contrato_venda_dir(), _uploads_dir() (+74 more)
...
Cohesion: 0.09
Nodes (32): _atomic_write(), _conflict_form_response(), _create_with_hook(), CrudModule, _delete_with_hook(), Any, FastAPI, HTMLResponse (+24 more)
...
Cohesion: 0.18
Nodes (11): BaseModel, LancamentoInvestimentoRead, ClienteRead, CompraRead, DocumentoProcuracaoRead, UsuarioRead, VeiculoRead, VendaRead (+3 more)
...
Cohesion: 0.12
Nodes (31): change_password(), create(), delete(), get(), get_by_username(), list_all(), Session, Usuário: enum de papel, model, schemas e CRUD. (+23 more)
...
Cohesion: 0.13
Nodes (16): BaseSettings, create_access_token(), decode_token(), get_settings(), Auth: settings, hash de senha e JWT (puro, sem FastAPI)., Settings, Token, TokenData (+8 more)
...
Cohesion: 0.20
Nodes (24): agregados_investidores(), create(), criar_lancamento_fechamento(), criar_lancamento_veiculo(), deletar_lancamento_veiculo(), delete(), _descricao_veiculo(), get() (+16 more)

### Community 85 - "auditoria.py"
Cohesion: 0.05
Nodes (54): Engine, create_test_engine(), Helpers for test database bootstrap., _reset_postgres_schema(), _run_migrations(), client(), TestClient, API compras: CRUD via TestClient. (+46 more)
...
Cohesion: 0.27
Nodes (17): Investidor, StatusVenda, TestClient, client(), Decimal, Venda, Fechamento financeiro de vendas., _seed_api() (+9 more)

### Community 95 - "core.py"
Cohesion: 0.28
...
## Knowledge Gaps
- **107 isolated node(s):** `Overview`, `Health Check`, `Rate Limiting`, `Login`, `Create User` (+102 more)
...
- **Why does `create_test_engine()` connect `auditoria.py` to `core.py`, `test_ui.py`, `__init__.py`, `core.py`?**
  _High betweenness centrality (0.067) - this node is a cross-community bridge._
- **Why does `_StubSchema` connect `core.py` to `perfis.py`?**
  _High betweenness centrality (0.066) - this node is a cross-community bridge._
- **Why does `_found()` connect `veiculos.py` to `auditoria.py`, `_found`, `setup.py`, `5c6914b729c6_index_fks_veiculo.py`, `test_venda_whatsapp.py`?**
  _High betweenness centrality (0.063) - this node is a cross-community bridge._
- **Are the 39 inferred relationships involving `_found()` (e.g. with `_veiculos_comprador_modal()` and `_veiculos_vendedor_modal()`) actually correct?**
