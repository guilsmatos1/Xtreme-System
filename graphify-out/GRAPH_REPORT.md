...
## Corpus Check
- 130 files · ~46,223 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1169 nodes · 2462 edges · 111 communities (103 shown, 8 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 152 edges (avg confidence: 0.72)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `4645a19f`
...
Cohesion: 0.05
Nodes (60): main(), Cria o primeiro admin: uv run python development/create_admin.py <user> <senha>., Engine, Jinja2Templates, db_session(), Session, Sessão isolada com schema migrado em Postgres ou SQLite local., _reset_rate_limiters() (+52 more)
...
### Community 5 - "core.py"
Cohesion: 0.20
Nodes (10): BaseModel, ClienteRead, CompraRead, DocumentoVeiculoRead, ImagemVeiculoRead, InvestidorRead, PerfilRead, UsuarioRead (+2 more)

### Community 6 - "core.py"
Cohesion: 0.08
Nodes (51): OrigemLancamento, TipoLancamento, TipoCliente, Papel, create(), delete(), get(), get_by_placa() (+43 more)
...
### Community 8 - "Usuario"
Cohesion: 0.07
Nodes (61): criar_usuario(), auditar(), Auditoria, AuditoriaRead, count(), _filtros(), get(), Any (+53 more)
...
## Knowledge Gaps
- **116 isolated node(s):** `Opportunity 1: Sale lifecycle invariants are not enforced`, `Opportunity 2: Upload validation trusts client metadata and unsafe paths`, `Opportunity 3: Deleting parent records leaves upload files behind`, `Opportunity 4: The rate limiter leaks per-IP buckets`, `Opportunity 5: Investor aggregates are computed in Python on every render` (+111 more)
...
- **Why does `Base` connect `Base` to `core.py`, `core.py`, `core.py`, `Usuario`, `Base`, `core.py`, `core.py`, `core.py`, `core.py`, `core.py`, `core.py`, `core.py`, `core.py`?**
  _High betweenness centrality (0.086) - this node is a cross-community bridge._
- **Why does `Usuario` connect `Usuario` to `_found`, `core.py`, `setup.py`, `core.py`, `Base`, `auditoria.py`, `perfis.py`, `test_auditoria.py`?**
  _High betweenness centrality (0.066) - this node is a cross-community bridge._
- **Why does `create_test_engine()` connect `create_test_engine` to `test_ui.py`, `route_factories.py`, `test_api_vendas.py`, `test_api_auth.py`, `test_venda_whatsapp.py`?**
...
- **What connects `Opportunity 1: Sale lifecycle invariants are not enforced`, `Opportunity 2: Upload validation trusts client metadata and unsafe paths`, `Opportunity 3: Deleting parent records leaves upload files behind` to the rest of the system?**
