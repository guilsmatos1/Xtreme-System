...
End-to-end workflow for authoring and maintaining Playwright tests with `coding--debug--playwright-cli`. Every `coding--debug--playwright-cli` action emits the equivalent Playwright TypeScript, and that generated code is the raw material for every test. The sections below can be used independently:
...
Plan / generate / heal lean on the same mechanic: run `npx playwright test --debug=cli` in the background, then `coding--debug--playwright-cli attach tw-XXXX` to drive the paused page interactively. See [playwright-tests.md](playwright-tests.md) for the debug/attach mechanics.

---

## 0. How generation works

Every action you perform with `coding--debug--playwright-cli` generates corresponding Playwright TypeScript code. This code appears in the output and can be copied directly into your test files.

```bash
# Start a session
coding--debug--playwright-cli open https://example.com/login
...
test('login flow', async ({ page }) => {
  // Generated code from coding--debug--playwright-cli session:
...
Use `coding--debug--playwright-cli generate-locator <target>` to produce the locator expression for the assertion, and the snapshot/eval commands to capture the expected value.
...
coding--debug--playwright-cli --raw generate-locator e5
# getByRole('button', { name: 'Submit' })

# Capture expected text content for toHaveText
coding--debug--playwright-cli --raw eval "el => el.textContent" e5

# Capture expected input value for toHaveValue/toBeEmpty
coding--debug--playwright-cli --raw eval "el => el.value" e5

# Capture expected aria snapshot for toMatchAriaSnapshot/toBeChecked
# (whole page, or use a ref to scope to a region)
coding--debug--playwright-cli --raw snapshot
coding--debug--playwright-cli --raw snapshot e5
```
...
coding--debug--playwright-cli attach tw-XXXX
```

Resume so the seed runs, then probe the app:

```bash
coding--debug--playwright-cli resume                   # resume so that seed test runs fully
coding--debug--playwright-cli snapshot                 # inventory of interactive elements
coding--debug--playwright-cli click e5                 # follow a flow
coding--debug--playwright-cli eval "location.href"     # read URL / state
coding--debug--playwright-cli show --annotate          # ask the user to point at something
```
...
**Important**: Do not just open the app url with coding--debug--playwright-cli, always go through the test to capture any custom setup done there.
...
**Do not** just open the app url with coding--debug--playwright-cli, always go through the test to capture any custom setup done there.

Walk the scenario's `Steps:` one by one with `coding--debug--playwright-cli`, treating the spec as the plan and the live app as the source of truth. If a step is vague ("click the button" — which button?), references an element that no longer exists, or contradicts the app's actual behaviour, use your judgement: update the spec to match what the app really does, then keep going. Editing the spec mid-generation is expected.

Every action prints the equivalent Playwright TypeScript (see [How generation works](#0-how-generation-works)):

```bash
coding--debug--playwright-cli snapshot                         # find refs
coding--debug--playwright-cli fill e3 "John Doe"               # -> page.getByRole('textbox', {...}).fill(...)
coding--debug--playwright-cli press Enter
coding--debug--playwright-cli click e7
```
...
```bash
coding--debug--playwright-cli snapshot                # did the element change / move / rename?
coding--debug--playwright-cli console                 # app-side errors?
coding--debug--playwright-cli requests                # failed request? wrong payload?
coding--debug--playwright-cli show --annotate         # ask the user to point somewhere
```

Common causes: selector drift, new wrapper element, label/ARIA rename, timing (transition, async load), assertion text updated in the app, test data leaking between runs.

Rehearse the corrected interaction with `coding--debug--playwright-cli` — the generated code in the output is what you paste back into the test.
