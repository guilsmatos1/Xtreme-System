...
- Never delete or recreate existing worktrees/branches without explicit user approval. The one standing exception is the `failed` path: when a worker reports `--phase failed`, the helper removes that issue's worktree and branch (`orca worktree rm --force`) so the issue is retryable. This is deliberate and destroys the failed attempt's changes.
